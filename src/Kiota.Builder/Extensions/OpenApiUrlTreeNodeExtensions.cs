...
public static partial class OpenApiUrlTreeNodeExtensions
{
    ...
    private static IEnumerable<OpenApiParameter> GetParametersForPathItem(OpenApiPathItem pathItem, string nodeSegment)
    {
        return pathItem.Parameters
                    ...
                    .Where(x => nodeSegment.Contains($"{{{x.Name}}}", StringComparison.OrdinalIgnoreCase) || nodeSegment.Contains($"{{+{x.Name}}}", StringComparison.OrdinalIgnoreCase) || nodeSegment.Contains($"{{#{x.Name}}}", StringComparison.OrdinalIgnoreCase));
    }
